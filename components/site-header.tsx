...
const links = [["Projects", "/projects"], ["About", "/about"], ["Blog", "/blog"], ["Contact", "/contact"]];
const serviceGroups = [
  ...
  return <>
    ...
    <header className="header">
      <div className="container nav-wrap">
        <Link href="/" className="brand brand-image" aria-label="Hot & Cool Services home"><Image src={images.logoRectangle} alt="Hot & Cool Services" width={220} height={70} priority/></Link>
        <nav className="desktop-nav" aria-label="Primary navigation">
          <div
            className={`nav-services ${megaOpen ? "is-open" : ""}`}
          ...
          >
            <button className="nav-services-trigger" type="button" aria-expanded={megaOpen} aria-controls="services-mega-menu" onClick={() => setMegaOpen((value) => !value)}>
        ...
        <button className="menu-button" onClick={() => setMobileOpen((value) => !value)} aria-expanded={mobileOpen} aria-controls="mobile-navigation" aria-label={mobileOpen ? "Close navigation" : "Open navigation"}>{mobileOpen ? <X/> : <Menu/>}</button>
      </div>
      {mobileOpen && <nav className="mobile-nav" id="mobile-navigation" aria-label="Mobile navigation">
        <button className="mobile-services-trigger" type="button" onClick={() => setMobileServicesOpen((value) => !value)} aria-expanded={mobileServicesOpen} aria-controls="mobile-services-list">
        ...
        {links.map(([label, href]) => <Link onClick={closeMobile} href={href} key={href}>{label}</Link>)}
